...
attract_all_column_names = ["Gene_name", "Gene_id", "Mutated", "Organism",
                            ...
                            "Database", "Pubmed", "Experiment", "Family",
                            "Matrix_id", "attractScore"]
...
attract_column_names = [attract_all_column_names[i]
                                        for i in attract_columns_of_interest]
attract_column_descriptions = [attract_all_column_descriptions[i]
                                        for i in attract_columns_of_interest]
...
def attract_data_load(rna_info, out=None):
    ...
    with open(attract_protein_file_path) as handle:
        ...
        assert (columns == ["Gene_name", "Gene_id", "Mutated", "Organism",
                            "Motif", "Len", "Experiment_description",
                            "Database", "Pubmed", "Experiment_description",
        ...
        while protein_columns != ['']:
            ...
            if protein_columns[3] != "Homo_sapiens":
                protein_columns = (handle.readline()
                                    .replace("\n", "")
                                    .split('\t')
                                  )
                continue
